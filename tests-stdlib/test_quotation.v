...
Elpi Query lp:{{ std.do! [
  ...
  (pi u\ X u = app[global GR _, u]) % Rocq <= 9.1
  ;
  (pi u\ X u = fun _ _ v\ fun _ _ w\ app[global GR _, u, v, w]), % Rocq > 9.1
  std.assert! (K = global GR _, coq.locate "Ord" GR) "not the right constructor"
]
}}.
