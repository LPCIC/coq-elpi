...
Module Type MutualIsKExpected.
  Include MutualBase.
  Definition tree_is_node : tree -> bool :=
  fun i : tree => match i with
                            | node _ => true
                            end.
  Definition forest_is_empty : forest -> bool :=
  fun i : forest =>
  match i with
  | empty => true
  | cons _ _ => false
  end.
  Definition forest_is_cons : forest -> bool :=
  fun i : forest =>
  match i with
  | empty => false
  | cons _ _ => true
...
Module Type MutualProjKExpected.
  Include MutualBase.
  Definition tree_projnode1 : forest -> tree -> forest :=
  fun (_ : forest) (i : tree) =>
  match i with
  | node f0 => f0
  end.
  Definition forest_projcons1 : tree ->
       forest -> forest -> tree :=
  ...
  Definition forest_projcons2 : tree ->
...
Module Type ParametrizedMutualIsKExpected.
  Include ParametrizedMutualBase.
  Definition ptree_is_pnode : forall A : Type, ptree A -> bool :=
  fun (A : Type) (i : ptree A) =>
  match i with
  | pnode _ _ _ => true
  end.
  Definition pforest_is_pempty : forall A : Type, pforest A -> bool :=
  fun (A : Type) (i : pforest A) =>
  match i with
  | pempty _ => true
  | pcons _ _ _ => false
  end.
  Definition pforest_is_pcons : forall A : Type, pforest A -> bool :=
  fun (A : Type) (i : pforest A) =>
  match i with
  | pempty _ => false
  | pcons _ _ _ => true
...
Module Type ParametrizedMutualProjKExpected.
  Include ParametrizedMutualBase.
  Definition ptree_projpnode1 : forall A : Type,
       A ->
       pforest A ->
       ptree A -> A :=
  ...
  Definition ptree_projpnode2 : forall A : Type,
       A ->
       pforest A ->
       ptree A -> pforest A :=
  fun (A : Type) (_ : A) (_ : pforest A)
  ...
  Definition pforest_projpcons1 : forall A : Type,
       ptree A ->
       pforest A ->
       pforest A -> ptree A :=
  ...
  Definition pforest_projpcons2 : forall A : Type,
       ptree A ->
       pforest A ->
       pforest A -> pforest A :=
...
Module MutualIsK <: MutualIsKExpected.
  ...
  Example tree_is_node_computes : tree_is_node (node empty) = true.
  Proof. vm_compute. reflexivity. Qed.

  Example forest_is_empty_computes : forest_is_empty empty = true.
  Proof. vm_compute. reflexivity. Qed.

  Example forest_is_cons_computes : forest_is_cons (cons (node empty) empty) = true.
  Proof. vm_compute. reflexivity. Qed.

  Example forest_is_empty_rejects_cons : forest_is_empty (cons (node empty) empty) = false.
  Proof. vm_compute. reflexivity. Qed.
End MutualIsK.
...
Module MutualProjK <: MutualProjKExpected.
  ...
  Example tree_projnode1_computes :
    tree_projnode1 empty (node (cons (node empty) empty)) = cons (node empty) empty.
  Proof. vm_compute. reflexivity. Qed.

  Example forest_projcons1_computes :
    forest_projcons1 (node empty) empty (cons (node empty) empty) = node empty.
  Proof. vm_compute. reflexivity. Qed.
...
Module ParametrizedMutualIsK <: ParametrizedMutualIsKExpected.
  ...
  Example ptree_is_pnode_computes :
    ptree_is_pnode nat (pnode nat 2 (pempty nat)) = true.
  Proof. vm_compute. reflexivity. Qed.

  Example pforest_is_pempty_computes : pforest_is_pempty nat (pempty nat) = true.
  Proof. vm_compute. reflexivity. Qed.

  Example pforest_is_pcons_computes :
    pforest_is_pcons nat (pcons nat (pnode nat 2 (pempty nat)) (pempty nat)) = true.
  Proof. vm_compute. reflexivity. Qed.
...
Module ParametrizedMutualProjK <: ParametrizedMutualProjKExpected.
  ...
  Example ptree_projpnode1_computes :
    ptree_projpnode1 nat 0 (pempty nat) (pnode nat 2 (pempty nat)) = 2.
  Proof. vm_compute. reflexivity. Qed.

  Example ptree_projpnode2_computes :
    ptree_projpnode2 nat 0 (pempty nat) (pnode nat 2 (pcons nat (pnode nat 3 (pempty nat)) (pempty nat))) =
    pcons nat (pnode nat 3 (pempty nat)) (pempty nat).
  Proof. vm_compute. reflexivity. Qed.

  Example pforest_projpcons1_computes :
    pforest_projpcons1 nat (pnode nat 0 (pempty nat)) (pempty nat)
      (pcons nat (pnode nat 2 (pempty nat)) (pempty nat)) = pnode nat 2 (pempty nat).
  Proof. vm_compute. reflexivity. Qed.

  Example pforest_projpcons2_computes :
    pforest_projpcons2 nat (pnode nat 0 (pempty nat)) (pempty nat)
