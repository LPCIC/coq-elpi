...
let set_solver_mode = function
  | ARm -> fun _ l -> takeover_rm l
  | AAdd -> fun _ l -> takeover_add l
  | ASet  -> fun x l -> takeover false l x
  | ANone -> fun x l -> takeover true l x
  | AAll  -> fun x l -> takeover false l x
let solver_register x = takeover false [] x
let solver_activate s = set_solver_mode AAll s []
let solver_deactivate s = set_solver_mode ANone s []
   


module Coq_elpi_class_tactics_hacked = struct
  (************************************************************************)
  ...
  *)
  ...
  let resolve_all_evars depth unique env p oevd fail =
    ...
    let split_solver = List.map (handle_takeover Search.typeclasses_resolve env oevd) split in
