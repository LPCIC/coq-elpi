...
module Search = struct
  ...
  (** Local hints *)
  let autogoal_cache = Summary.ref ~name:"autogoal_cache1"
...
let assert_same_generated_TC = Goptions.declare_bool_option_and_ref ~depr:(Deprecation.make ()) ~key:["assert_same_generated_TC"] ~value:false 
...
let resolve_all_evars depth unique env p oevd do_split fail =
  ...
  let rec docomp (evd: evar_map) : ('a * Intpart.set) list -> evar_map = function
    | [] ->
      let () = ppdebug 2 (fun () ->
          str"Final evar map: " ++
...
let solve_inst env evd filter unique fail =
  let ((), sigma) = Hints.wrap_hint_warning_fun env evd begin fun evd ->
    ...
    unique env evd filter fail true
  end in
  sigma
