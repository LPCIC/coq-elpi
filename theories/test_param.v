...
Elpi Query param "env-add-param {{@unit}} ""unitR""".
Elpi Query param "env-add-param {{@nat}} ""natR""".

Inductive fin : nat -> Type :=
    FO : fin 0 | FS : forall n : nat, fin n -> fin (S n).
Elpi Query param "env-add-param {{@fin}} ""finR"")".

Fixpoint fin_length  n (v : fin n) :=
  match v with FO => 0 | FS _ w => S (fin_length _ w) end.
Elpi Query param "env-add-param {{@fin_length}} ""fin_lengthR"")".

Inductive vec (A : Type) : nat -> Type :=
    nil : vec A 0 | cons : A -> forall n : nat, vec A n -> vec A (S n).
Elpi Query param "env-add-param {{@vec}} ""vecR"")".

Fixpoint vec_length (A : Type) n (v : vec A n) :=
  match v with nil _ => 0 | cons _ _ _ w => S (vec_length _ _ w) end.
Elpi Query param "env-add-param {{@vec_length}} ""vec_lengthR"")".


Elpi Query param "env-add-param {{@list}} ""listR""".
Elpi Query param "env-add-param {{@eq}} ""eqR""".

Fixpoint plus' m n := match n with 0 => m | S n => S (plus' m n) end.
Elpi Query param "env-add-param {{@plus'}} ""plus'R""".
Elpi Query param "env-add-param {{@plus}} ""plusR""".

Definition test m n p q r := m + n + p + q + r.
Elpi Query param "env-add-param {{@test}} ""testR""".

Definition vec_length_type := forall (A : Type) (n : nat), vec A n -> nat.

Elpi Query param "env-add-param {{@vec_length_type}} ""vec_length_typeR"")".

Definition vec_length_rec (vec_length : vec_length_type)
  (A : Type) n (v : vec A n) :=
  match v with nil _ => 0 | cons _ _ _ w => S (vec_length _ _ w) end.
Elpi Query param "env-add-param {{@vec_length_rec}} ""vec_length_recR"")".

Elpi Query param "with-TC-param (param {{O}} X Y)".
Elpi Query param "with-TC-param (param {{S (S 0)}} X Y)".

Fail Elpi Query param "param-const {{@eq_refl}} _ _ _ _ _ _".
...
Elpi Query param "env-add-param {{@nat2nat}} ""nat2natR""".
Elpi Query param "env-add-param {{@nat2nat2nat}} ""nat2nat2natR""".
Elpi Query param "env-add-param {{@pred}} ""predR""".
Print predR.
Check (predR : nat2natR pred pred).

Fixpoint predn n := match n with 0 => 0 | S n => S (predn n) end.

Elpi Query param "env-add-param {{@predn}} ""prednR""".
...
Elpi Query param "env-add-param {{@bla}} ""blaR"")".
