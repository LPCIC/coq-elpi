    ...
    | Some (relocate, state, assignments, constraints) ->
       ...
       let typ =
        try
          ...
          let state, t, _ = Rocq_elpi_builtins.closed_ground_term.readback ~depth:0 Rocq_elpi_HOAS.(mk_coq_context ~options:(default_options()) state) constraints state t in
          Some (EConstr.to_constr ~abort_on_undefined_evars:false sigma t)
    ...
    let ext =
      Vernacextend.declare_dynamic_vernac_extend
        ...
        (fun loc0 args loc1 ?loc ~atts () ->
          ...
          match proof with
          | None -> Vernactypes.vtdefault (fun () ->
              Interp.run_program ~loc p ~atts ~syndata args)
          | Some Begin -> Vernactypes.vtopenproof (fun () -> Interp.run_program_open_proof ~loc p ~atts ~syndata args)
