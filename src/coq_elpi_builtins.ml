...
let purge_algebraic_univs csts t =
  ...
  let rec aux t =
    ...
    | Constr.Sort (Sorts.Type u) when not (Univ.Universe.is_level u) ->
        ...
        csts := add_universe_constraint new_csts u Universes.ULe v;
...
let coq_builtins = 
        ...
        let csts, bo =
          ...
          if opaque then csts, Some in_elpi_implicit
            ...
            | Some bo ->
                ...
                csts, Some bo
            | None -> csts, Some in_elpi_implicit in
        csts, ?: bo +? ty
    | `Var v ->
        let csts, ty =
          if ty = Discard then csts, None else
  ...
  (fun u1 u2 ~depth _ { custom_constraints = csts } ->
    ...
    add_universe_constraint csts u1 Universes.ULe u2, !: u1 +! u2)),
  DocAbove);
  ...
  (fun u1 u2 ~depth _ { custom_constraints = csts } ->
    let csts, u1 = force_univ ~depth csts u1 in
    let csts, u2 = force_univ ~depth csts u2 in
    add_universe_constraint csts u1 Universes.UEq u2, !: u1 +! u2)),
  DocAbove);
