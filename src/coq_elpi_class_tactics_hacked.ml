...
let hack_opt_i on () =
  ...
  match Goptions.OptionMap.find_opt on table with
  | Some { Goptions.opt_value = Goptions.IntValue x ; _ }  -> x
  | _ -> None

let hack_opt_b def on () =
  let table = Goptions.get_tables () in
  match Goptions.OptionMap.find_opt on table with
  | Some { Goptions.opt_value = Goptions.BoolValue x; _ } -> x
  | _ -> def
  
let typeclasses_depth_opt_name = ["Typeclasses";"Depth"]

let get_typeclasses_depth = hack_opt_i typeclasses_depth_opt_name
...
let typeclasses_limit_intros_name = ["Typeclasses";"Limit";"Intros"]

let get_typeclasses_limit_intros = hack_opt_b true typeclasses_limit_intros_name

let typeclasses_dependency_order_name = ["Typeclasses";"Dependency";"Order"] 
let get_typeclasses_dependency_order = hack_opt_b false typeclasses_dependency_order_name

let iterative_deepening_opt_name = ["Typeclasses";"Iterative";"Deepening"]
let get_typeclasses_iterative_deepening = hack_opt_b false iterative_deepening_opt_name
...
(** [typeclasses_filtered_unif] governs the unification algorithm used by type
    ...
    used. *)
let typeclasses_filtered_unification_name = ["Typeclasses";"Filtered";"Unification"]
let get_typeclasses_filtered_unification = hack_opt_b false typeclasses_filtered_unification_name
...
module Search = struct
  ...
  (** Local hints *)
  let autogoal_cache = Summary.ref ~name:"autogoal_cache_"
...
let { Goptions.get = assert_same_generated_TC } =
  Goptions.declare_bool_option_and_ref ~key:["assert_same_generated_TC"] ~value:false ()
...
let same_solution evd1 evd2 i =
  ...
  try (
    let Evd.EvarInfo t1 = Evd.find evd1 i in
    let t1 = Evd.evar_body t1 in 
    let Evd.EvarInfo t2 = Evd.find evd2 i in
    let t2 = Evd.evar_body t2 in 
    match t1, t2 with 
...
let resolve_all_evars depth unique env p oevd fail =
  ...
  let split = List.map (handle_takeover env oevd) split in
...
let resolve_typeclass_evars depth unique env evd filter fail : Evd.evar_map =
  let evd =
