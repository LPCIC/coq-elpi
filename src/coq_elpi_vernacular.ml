...
and src_string = {
  ...
  val accumulate_to_db : qualified_name -> Compile.compilation_unit list -> Names.Id.t list -> local:bool -> unit
...
let add_to_db program code vars local =
  Lib.add_anonymous_leaf (in_db { program; code; local; vars })
...
let () = Coq_elpi_builtins.set_accumulate_to_db (Programs.ensure_initialized, (fun n x vs ~local -> Programs.accumulate_to_db n [x] vs ~local), (fun () -> Option.get !current_program_begin_run))
open Programs
