...
let load_api apis =
  ...
  Format.fprintf fmt "%% See also %a@\n"
     (E.RawPp.list Format.pp_print_string " ") apis;
  E.BuiltIn.document fmt builtin_declarations;
...
let run ~static_check ?(flags = cc_flags ()) program_ast query =
  ...
  let query =
    ...
    | `Fun query_builder -> E.RawQuery.compile program query_builder in
  E.Setup.trace [];
  if static_check then run_static_check query;
  E.Setup.trace !trace_options;
  E.Execute.once ~max_steps:!max_steps (EC.link query)
;;
...
let run_and_print ~print ~static_check ?flags program_ast query_ast =
  ...
  | E.Execute.Success {
    ...
    let clauses_to_add =
      E.State.get Coq_elpi_builtins.clauses_for_later
        state in
...
let typecheck ?(program = current_program ()) () =
  let program_ast = get (snd program) in
  let query_ast = parse_goal (E.Ast.Loc.initial "(typecheck)") "true." in
  let header, api = get_header () in
  let program =
...
let print (_,name as prog) args = 
  let args, fname =
    ...
    | x :: xs -> xs, x in
  let args = List.map E.RawOpaqueData.of_string args in
  let program_ast = get (snd prog) in
  let query_ast = parse_goal (E.Ast.Loc.initial "(print)") "true." in
  let header, api = get_header () in
  let program =
    EC.program ~flags:EC.default_flags header (api :: program_ast) in
  let query = EC.query program query_ast in
  let quotedP, _  = E.Quotation.quote_syntax query in
  ...
  let q ~depth state =
    ...
    let q = ET.mkApp (ET.Constants.from_stringc "main-quoted")
      ...
      [E.RawOpaqueData.of_string fname; EU.list_to_lp_list args] in
    state, (loc,q) in
  let flags =
