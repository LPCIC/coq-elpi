...
(* elpi.vo loaded *)
let api_unit = Summary.ref ~name:"elpi-api" None
let in_elpi_api : API.Compile.compilation_unit -> Libobject.obj =
  Libobject.declare_object { Libobject.(default_object "ELPI-API") with
    Libobject.load_function = (fun _ (_,x) -> api_unit := Some x);
}
...
let init () =
  ...
  let e, _ = API.Setup.init ~builtins ~basedir:"."
...
let load_hoas_def apis =
  ...
  close_out oc;
  let api = unit_from_file ~elpi apis in
  api_unit := Some api;
...
let get_header () =
  let elpi = ensure_initialized () in
  match !api_unit with
  | None -> CErrors.user_err Pp.(str "Call Elpi Api first");
  | Some y -> elpi, y
...
let () = Coq_elpi_builtins.set_accumulate_to_db (fun n x -> Programs.accumulate_to_db n [x])
open Programs
