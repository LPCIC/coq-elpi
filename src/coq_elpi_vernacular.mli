...
val pr_prog_arg : prog_arg -> Pp.std_ppcmds

type program_kind = Command | Tactic

val set_current_program : ?kind:program_kind -> qualified_name -> unit

val run_program : Ploc.t * qualified_name -> prog_arg list -> unit
val run_in_program : ?program:(Ploc.t * qualified_name) -> Ploc.t * string -> unit
val print : Ploc.t * qualified_name -> string list -> unit

val run_tactic : Ploc.t * qualified_name -> Geninterp.interp_sign -> unit Proofview.tactic
val run_in_tactic : ?program:(Ploc.t * qualified_name) -> Ploc.t * string -> Geninterp.interp_sign -> unit Proofview.tactic
